...
import { explain_my_answer } from "@/agent/tools/explain-answer";
...
If the student asks why they got a quiz question wrong, or refers to "la question" / "le quiz", call \`explain_my_answer\` with the telegramUserId given at the start of the conversation. That tool is the only way to know which question they mean and what they answered — never reply that you lack the context without calling it first.
...
export const studentHelperAgent = new BuiltInAgent({
  ...
  tools: [list_lessons, get_lesson_transcript, explain_my_answer],
});
